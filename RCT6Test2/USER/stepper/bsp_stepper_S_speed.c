...
void Speed_Decision(void)
{
  ...
  static __IO uint32_t index = 0;
	...
	if(__HAL_TIM_GET_IT_SOURCE(&htim1, MOTOR_TIM_IT_CCx) != RESET)
	{
		/* 清除定时器中断 */
		__HAL_TIM_CLEAR_IT(&htim1, MOTOR_TIM_IT_CCx);
    ...
    if(i == 2)
    {
      ...
      switch(Stepper.status)
      {
        case ACCEL:
          ...
          /* 获取每一步的定时器计数值 */
          Stepper.pluse_time = (uint16_t)(T1_FREQ_3 / Speed.Form[index] / 2U);
          /* 步数位置索引递增 */
          index++;
          break;
        case DECEL:
      ...
      Stepper.pos++;
		...
		/**********************************************************************/
		/* 获取当前计数器数值 */
		uint32_t tim_count = __HAL_TIM_GET_COUNTER(&htim1);
		/* 计算下一次时间 */
...
bool Stepper_Move_S(int16_t start_speed, int16_t end_speed, float acc_time, int32_t step)
{
  ...
  if(Stepper.status != STOP)
    ...
    return false;
		...
		step=step*Motor3Factor;//新增的添加一个比例系数；
  if(step < 0)
  {
    step = -step;
//    MOTOR_DIR(CCW);
...
//    MOTOR_DIR(CW);
  ...
  /* 计算第一步的定时器参数 */
  Stepper.pluse_time = (uint16_t)(T1_FREQ_3 / Speed.Form[0] / 2U);

  /* 清零计数器 */
  __HAL_TIM_SET_COUNTER(&htim1, 0);
  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, Stepper.pluse_time);
  /* 使能定时器通道 */
